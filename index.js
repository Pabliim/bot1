...
async function starts() {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const ownerNumber = ["6391084396@s.whatsapp.net"] // GANTI NOMOR LU
			const adminbotnumber = ["6391084396@s.whatsapp.net"] // GANTI NOMOR LU
			const frendsowner = ["6391084396@s.whatsapp.net"] // GAUSAH DIGANTI!!!
			...
			switch(command) {
				...
				case 'sticker':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`❌ Falha ao converter vídeo em adesivos ${tipe} ke stiker`)
							})
							.on('end', function () {
								console.log('Finish')
								buff = fs.readFileSync(ran)
					...
					/*} else if ((isMedia || isQuotedImage) && colors.includes(args[0])) {
						...
						reply(`Envie fotos com legendas / adesivos ou tags de imagem que foram enviadas ${prefix}envie o gif ou foto com a legenda`)
					}
					break
			case 'dono':
                 client.sendMessage(from, {displayname: "Pablo", vcard: vcard}, MessageType.contact, { quoted: mek})
                 client.sendMessage(from, 'Esse é meu namorado, meu dono>',MessageType.text, { quoted: mek} )
                 break
                 case 'fitnah':	
				case 'fake':          
